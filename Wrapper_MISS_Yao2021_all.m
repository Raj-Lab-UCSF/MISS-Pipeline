...
load([matdir filesep 'CellDensity_Yao2021_all.mat'],'outstruct','ng_param_list','lambda',... %saving cell mapping output
    'missmethod','infmethod','geneinds','classkey');
...
% types = [10:28,35,39,40,42];
% types = [1,9,31,34,36,41];
types = 42;
cmap_range = repmat({[1 1 0;0 1 0]},1,length(types));
...
% voxthresh = [0.7*ones(1,19), 0.7*ones(1,4)];
voxthresh = 0.65*ones(1,length(types));
