function metric_struct = TauCalc_mod(outstruct,idx,cell_names,cell_inds,ranks,directory)
...
for i = 1:length(structList)
    ...
    curvox = predvecs(voxinds,cell_inds);
...
%             mndists(j-9,k) = -1;
            ...
            if mean(curslc(paleoinds))>mean(curslc(neoinds))
                offtarget(j-9,k) = mean(curslc(paleoinds)) / (mean(curslc(neoinds))+mean(curslc(paleoinds)));
            else
                offtarget(j-9,k) = 0;
            end
        end
...
%     n_nosig(j) = length(find(nosig));
    penalty(j) = sum(squeeze(offtarget(j,:)));
    curmns(nosig) = [];
    ...
    tau_slice(j) = (1-(penalty(j)/7))*tau_per;
...
if sum(lranks) == 0
    ...
    tau = (1-(sum(penalty)/(length(ranks)*length(10:length(maplocs)))))*tau;
end
